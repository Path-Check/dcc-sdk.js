/**
 * Copy of @cose-js with added support for RSASSA-PSS
 */
...
function subtle() {
  return crypto.webcrypto ? crypto.webcrypto.subtle : window.crypto.subtle;
}
...
const COSEAlgToNodeAlg = {
  ...
  'PS256': { 'sign': 'RSA-PSS', 'digest': 'SHA-256', saltLength: 32  },
  'PS384': { 'sign': 'RSA-PSS', 'digest': 'SHA-384', saltLength: 48  },
  'PS512': { 'sign': 'RSA-PSS', 'digest': 'SHA-512', saltLength: 64  },
...
function doSign (SigStructure, signer, alg) {
  return new Promise((resolve, reject) => {
    ...
    if (AlgFromTags[alg].sign.startsWith('ES')) {
      ...
      var keyData = {
          kty: 'RSA',
          e: verifier.key.e.toString('base64').replace(/=/g, "").replace(/\+/g, "-").replace(/\//g, "_"),
          n: verifier.key.n.toString('base64').replace(/=/g, "").replace(/\+/g, "-").replace(/\//g, "_")
      };

      subtle().importKey('jwk', keyData, algo, false, ['verify'])
      .then (function (importedKey){
          subtle().sign(algo, importedKey, ToBeSigned).then(sig => {
            resolve(sig);
        }).catch(function(err) {
          reject(err);
        });
...
function doVerify (SigStructure, verifier, alg, sig) {
  return new Promise((resolve, reject) => {
    ...
    if (AlgFromTags[alg].sign.startsWith('ES')) {
      ...
      subtle().importKey('jwk', keyData, algo, false, ['verify'])
      .then (function (importedKey){
          subtle().verify(algo, importedKey, sig, ToBeSigned).then(verified => {
            if (verified)
              resolve();
            else
              throw new Error('Signature missmatch');
...
exports.verify = function (payload, verifier, publicKeyPem, options) {
  options = options || {};
