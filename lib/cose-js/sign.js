...
async function doVerify (SigStructure, verifier, alg, sig) {
  ...
  if (verifier.key.e) {
    keyData["e"] = verifier.key.e.toString('base64').replace(/=/g, "").replace(/\+/g, "-").replace(/\//g, "_");
  }
  if (verifier.key.n) {
    keyData["n"] = verifier.key.n.toString('base64').replace(/=/g, "").replace(/\+/g, "-").replace(/\//g, "_");
  }
  if (verifier.key.x) {
    keyData["x"] = verifier.key.x.toString('base64').replace(/=/g, "").replace(/\+/g, "-").replace(/\//g, "_");
  }
  if (verifier.key.y) {
    keyData["y"] = verifier.key.y.toString('base64').replace(/=/g, "").replace(/\+/g, "-").replace(/\//g, "_");
  }

  const importedKey = await subtle().importKey('jwk', keyData, algo, false, ['verify']);
