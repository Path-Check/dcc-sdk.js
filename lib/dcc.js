...
import * as oldBase45 from 'base45-js'; // Bugs in production, using latest dev version
import * as newBase45 from './base45-js'; // Bugs in production, using latest dev version
...
function getParamsFromPEM(publicKeyPem) {
  ...
  if (cert.publicKey.oid === RSA_IOD) {
    ...
    const keyMod = Buffer.from(pk.slice(8, 8+385));
    const keyExp = Buffer.from(pk.slice(8+385+2,8+385+2+4));
