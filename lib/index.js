...
function getTXT(url) {
    ...
    client.setRequestHeader("Accept", "text/plain;charset=UTF-8");
    client.send();
 ...
 */
async function makeCWT(payload) {
  var issue = new Date();
  ...
  let cwt = new Map();
  cwt.set(CWT_ISSUED_AT, Math.round(issue.getTime()/1000));
...
async function getIssuerKeyId(coseContent) {
  ...
  let [protec, unprotec, payload, signature] = cborObj.value;
  let protectedObj = cbor.decode(protec);
  let algorithm = protectedObj.get(COSE_ALG_TAG);
  let kid = new Uint8Array(protectedObj.get(COSE_KID_TAG));
 
  let decodedPayload = cbor.decode(payload); 
  let issuer;
  if (cbor.decode(payload) instanceof Map) {
    issuer = decodedPayload.get(CWT_ISSUER);
  }
  return {alg: algorithm, kid: kid, iss: issuer};
}

async function resolveKeyIDPEM(kID, issuer) {
  var b64encoded = btoa(String.fromCharCode.apply(null, kID)).replace(/\+/g,'-').replace(/\//g,'_').replace(/\=+$/m,'');
  console.log("KID: ", b64encoded);
  //if (issuer === "AT") {
  //  atKey = getTXT('https://dgc.a-sit.at/ehn/cert/'+b64encoded)
  //  console.log(atKey);
  //}
}
...
async function verifyAndReturnPayload(coseContent, publicKeyPem) {
  ...
  try {
    publicKeyPem = await resolveKeyIDPEM(keyID.kid, keyID.iss);

    if (publicKeyPem != null)
      keyParams = getParamsFromPEM(publicKeyPem);
  } catch (err) {
    console.warn("Failed loading key ", keyID, err);
  }
...
module.exports = {
  sign, verify, pack, unpack, signAndPack, unpackAndVerify, signAndPack32, signAndPack45, makeCWT, debug
};
