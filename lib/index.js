...
function getTXT(url) {
    ...
    client.open('GET', 'http://dgc.a-sit.at/ehn/cert/WTOXYrYS47o', false);
...
async function getKeyID(coseContent) {
  let something = cbor.decode(new Uint8Array(coseContent));
  return new Uint8Array(cbor.decode(something.value[0]).get(4));
}
...
async function verifyAndReturnPayload(coseContent, publicKeyPem) {
  ...
  const kID = await getKeyID(coseContent);

  if (kID.toString() !== verifier.key.kid.toString()) {
    var b64encoded = btoa(String.fromCharCode.apply(null, kID)).replace(/\+/g,'-').replace(/\//g,'_').replace(/\=+$/m,'');
    //console.log(getTXT('https://dgc.a-sit.at/ehn/cert/'+b64encoded));
    throw new Error("Public Key not found: " + kID + " " + b64encoded);
  }
...
module.exports = {
  sign, verify, pack, unpack, signAndPack, unpackAndVerify, signAndPack32, signAndPack45, makeCWT
};
