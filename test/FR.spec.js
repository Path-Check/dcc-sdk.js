...
describe('FR', async () => {
  ...
  it('should verify FR_2DCode_raw_vaccin_ok', async () => {
    ...
    expect(cwtPayload).not.to.be.null;
    expect(cwtPayload).not.to.be.undefined;
    expect(await parseCWT(cwtPayload.contents)).to.eql({'ver': '1.0.0', 'nam': {'fn': 'Dupond', 'fnt': 'DUPOND', 'gn': 'Marie', 'gnt': 'MARIE'}, 'dob': '1962-07-01', 'v': [{'tg': '840539006', 'vp': '1119305005', 'mp': 'EU/1/20/1507', 'ma': 'ORG-100031184', 'dn': 2, 'sd': 2, 'dt': '2021-01-05', 'co': 'FR', 'is': 'IN', 'ci': 'dgci:V1:FR:C51AOQW7CQMFW7WLIWVGADQY6:70'}]});
  });
  ...
  it('should verify FR_2DCode_raw_recovery_ok', async () => {
    ...
    expect(cwtPayload).not.to.be.null;
    expect(cwtPayload).not.to.be.undefined;
    expect(await parseCWT(cwtPayload.contents)).to.eql({'ver': '1.0.0', 'nam': {'fn': 'Dupond', 'fnt': 'DUPOND', 'gn': 'Marie', 'gnt': 'MARIE'}, 'dob': '1962-07-01', 'r': [{'tg': '840539006', 'fr': '2021-05-02', 'co': 'FR', 'is': 'Laboratory', 'df': '2021-05-02', 'du': '2023-05-02', 'ci': 'dgci:V1:FR:ME8M4JGYO2UKFHKKMQO8KW9QP:36'}]});
  });
