...
describe('SK', async () => {
  it('should verify SK_2DCode_raw_6', async () => {
    ...
    expect(await parseCWT(cwtPayload)).to.eql({'ver': '1.0.0', 'nam': {'fn': 'Michal', 'gn': 'Piško', 'fnt': 'MICHAL', 'gnt': 'PISKO'}, 'dob': '1999-12-24', 'r': [{'tg': '840539006', 'fr': '2021-04-21', 'co': 'SK', 'is': 'National Health Information Center, Slovakia', 'df': '2021-05-19', 'du': '2021-10-21', 'ci': 'urn:uvci:01:SK:A7C71100B145444B827299717EB2FFAE'}]});
  });
  ...
  it('should verify SK_2DCode_raw_1', async () => {
    ...
    expect(cwtPayload).not.to.be.null;
    expect(cwtPayload).not.to.be.undefined;
    expect(await parseCWT(cwtPayload)).to.eql({'ver': '1.0.0', 'nam': {'fn': 'Šimon', 'gn': 'Petrovič Lužičinský', 'fnt': 'SIMON', 'gnt': 'PETROVIC<LUZICINSKY'}, 'dob': '1990-01-01', 'v': [{'tg': '840539006', 'vp': '1119305005', 'mp': 'EU/1/20/1528', 'ma': 'ORG-100030215', 'dn': 1, 'sd': 2, 'dt': '2021-05-14', 'co': 'SK', 'is': 'National Health Information Center, Slovakia', 'ci': 'urn:uvci:01:SK:C0DA0FF1D1EE4643A81F953778520E0A'}]});
  });
  ...
  it('should verify SK_2DCode_raw_2', async () => {
    ...
    expect(cwtPayload).not.to.be.null;
    expect(cwtPayload).not.to.be.undefined;
    expect(await parseCWT(cwtPayload)).to.eql({'ver': '1.0.0', 'nam': {'fn': 'Sčasný', 'gn': 'Svätozár', 'fnt': 'SCASNY', 'gnt': 'SVAETOZAR'}, 'dob': '1984-09-27', 'v': [{'tg': '840539006', 'vp': '1119349007', 'mp': 'EU/1/20/1528', 'ma': 'ORG-100030215', 'dn': 1, 'sd': 2, 'dt': '2021-05-18', 'co': 'SK', 'is': 'National Health Information Center, Slovakia', 'ci': 'urn:uvci:01:SK:4DC1082B01524B868C87BE23345B3587'}]});
  });
  ...
  it('should verify SK_2DCode_raw_3', async () => {
    ...
    expect(cwtPayload).not.to.be.null;
    expect(cwtPayload).not.to.be.undefined;
    expect(await parseCWT(cwtPayload)).to.eql({'ver': '1.0.0', 'nam': {'fn': 'Prism', 'gn': 'Róbert', 'fnt': 'PRISM', 'gnt': 'ROBERT'}, 'dob': '1955-04-21', 'v': [{'tg': '840539006', 'vp': '1119305005', 'mp': 'EU/1/21/1529', 'ma': 'ORG-100001699', 'dn': 1, 'sd': 2, 'dt': '2021-03-30', 'co': 'SK', 'is': 'National Health Information Center, Slovakia', 'ci': 'urn:uvci:01:SK:AD49A022B66F43189238DC38B78D8027'}, {'tg': '840539006', 'vp': '1119305005', 'mp': 'EU/1/21/1529', 'ma': 'ORG-100001699', 'dn': 2, 'sd': 2, 'dt': '2021-05-19', 'co': 'SK', 'is': 'National Health Information Center, Slovakia', 'ci': 'urn:uvci:01:SK:AD49A022B66F43189238DC38B78D8027'}]});
  });
  ...
  it('should verify SK_2DCode_raw_8', async () => {
    ...
    expect(cwtPayload).not.to.be.null;
    expect(cwtPayload).not.to.be.undefined;
    expect(await parseCWT(cwtPayload)).to.eql({'ver': '1.0.0', 'nam': {'fn': 'Skywalkerová', 'gn': 'Nina', 'fnt': 'SKYWALKEROVA', 'gnt': 'NINA'}, 'dob': '1978-05-21', 't': [{'tg': '840539006', 'tt': 'LP6464-4', 'tr': '260415000', 'sc': '2021-05-20T12:34:56+00:00', 'dr': '2021-05-20T12:45:01+00:00', 'tc': 'Poliklinika Mýtna', 'co': 'SK', 'is': 'National Health Information Center, Slovakia', 'ci': 'urn:uvci:01:SK:E2FA2860F180475B99CFD15745AC12E4'}]});
  });
