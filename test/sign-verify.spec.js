...
describe('EU DCC', function() {
  ...
  it('should Verify the json', async () => {
    ...
    expect(result.contents).to.eql('{ "Foo":1, "Bar":{ "Field1": "a value",   "integer":1212112121 }}');
  });
  ...
  it('should Verify the json from Austria', async () => {
    ...
    const result = await unpackAndVerify(signed);
    expect(result.status).to.be.equals("verified");
    expect(await parseCWT(result.contents)).to.eql(TEST_PAYLOAD_AT);
  });
  ...
  it('should Verify the json from Sweden', async () => {
    ...
    const result = await unpackAndVerify(signed);
    expect(result.status).to.be.equals("verified");
    expect(await parseCWT(result.contents)).to.eql(TEST_PAYLOAD_SE);
  });
  ...
  */
  ...
  it('should Unpack, Debug and Verify UK Certs', async () => {
    ...
    const result = await unpackAndVerify(signed);
    expect(await parseCWT(result.contents)).to.eql(UK_PAYLOAD);
  });
