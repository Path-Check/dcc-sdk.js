const {sign, verify, pack, unpack, signAndPack, unpackAndVerify, signAndPack32, signAndPack45, makeCWT, parseCWT} = require('../lib/index');
const base45 = require('../lib/base45-js');
...
test('Sign and Verify a Payload (JSON->COSE->JSON)', async () => {
  ...
  expect(await parseCWT(result)).toStrictEqual(TEST_PAYLOAD);
});
...
test('Base45', async () => {
  ...
  expect(backAndForth).toStrictEqual(data);
});
