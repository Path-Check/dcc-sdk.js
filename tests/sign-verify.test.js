...
test('Verify the json from Austria', async () => {
  ...
  expect(await parseCWT(cwtPayload)).toStrictEqual(TEST_PAYLOAD_AT);
});
...
test('Verify the json from Sweden', async () => {
  ...
  const cwtPayload = await unpackAndVerify(signed);
  expect(await parseCWT(cwtPayload)).toStrictEqual(TEST_PAYLOAD_SE);
});
...
test('Make CWT', async () => {
  ...
  expect(await parseCWT(cwtPayload)).toStrictEqual(TEST_PAYLOAD);
});
